...
public class ListaDuplamenteEncadeada<T> {
    ...
    public T get(int posicao) throws Exception {
        ...
        if(posicao <= metade) {
            ItemListaDuplamenteEncadeada<T> itemEncontradoNaPrimeiraMetade = primeiroItem;
            for (int i = 0; i <= posicao; i++) {
                if (i == posicao) {
                    return itemEncontradoNaPrimeiraMetade.getDado();
                } else {
                    itemEncontradoNaPrimeiraMetade = itemEncontradoNaPrimeiraMetade.getProximo();
                }
            }
            return itemEncontradoNaPrimeiraMetade.getDado();
        }

        ItemListaDuplamenteEncadeada<T> itemEncontradoNaSegundaMetade = ultimoItem;
       for (int i = tamanho-1; i >= posicao ; i--) {
           if (i == posicao) {
               return itemEncontradoNaSegundaMetade.getDado();
           } else {
               itemEncontradoNaSegundaMetade = itemEncontradoNaSegundaMetade.getAnterior();
           }
       }
       return null;
    }
